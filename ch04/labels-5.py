...
class main_wnd (QWidget):
    ...
    def ds_set_label1(self):
        ...
        istr = os.path.join(pstr, './image/labelImage.jpg')#빽슬레시, 슬레시 신경쓰지 않아도 join으로 경로 설정
        ...
        img_fstr = os.path.join(path_py_file, './image/labelImage.jpg') #빽슬레시, 슬레시 신경쓰지 않아도 join으로 경로 설정
        try:
            with open(img_fstr):
