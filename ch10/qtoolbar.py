...
class MW(QMainWindow):
    ...
    def create_actions(self, root_dir):
        icon0_path = os.path.join(root_dir, './image/icon0.png')
        icon1_path = os.path.join(root_dir, './image/icon1.png')

        # 툴바에 액션 추가
        self.button_action0 = QAction("action0", self)
        self.button_action0.setStatusTip("This is the first button on")
        ...
        self.button_action1.setStatusTip("This is the second button on")
        self.button_action1.setIcon(QIcon(icon1_path))
        self.button_action1.setShortcut('Ctrl+1')
        self.button_action1.triggered.connect(self.onAction0)
